  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center justify-center w-full h-full max-w-[1400px] touch-none">
        <button 
          className="absolute left-0 text-white/20 hover:text-white p-4 h-full flex items-center z-[3050] transition-colors group"
          onClick={(e) => { e.stopPropagation(); onPrev(); }}
        >
          <ChevronLeft size={32} strokeWidth={1} className="group-hover:scale-110 transition-transform md:w-12 md:h-12" />
        </button>
        ...
        >
          <img
            src={images[index]}
            className="max-w-[95vw] md:max-w-[85vw] max-h-[75vh] md:max-h-[85vh] object-contain shadow-[0_20px_50px_rgba(0,0,0,0.5)] pointer-events-none select-none"
            onClick={(e) => (e as any).stopPropagation()}
          />
        </motion.div>

        <button 
          className="absolute right-0 text-white/20 hover:text-white p-4 h-full flex items-center z-[3050] transition-colors group"
          onClick={(e) => { e.stopPropagation(); onNext(); }}
        >
          <ChevronRight size={32} strokeWidth={1} className="group-hover:scale-110 transition-transform md:w-12 md:h-12" />
        </button>
      </div>
